  ...
  function BookHeader() {
    return (
      <header className="d-flex justify-content-between align-items-center p-4">
        <div>
          <span className="brand-title bi bi-book"> Store</span>
        </div>

        <div className="d-flex">
          <button className="btn btn-danger ms-2">Register</button>
  ...
  function BookList({ books }) {
    return (
      <div className="bookList">
        <div className="row">
          {/* <div className="col-6">
            <h3 className="text-danger bg-info p-4 text-center">
              Best Selling Books
            </h3>
            <div className="row">
              {books.map((book) => (
          ...
          <div className="col">
            <div className="">
              <h3 className="text-danger bg-warning p-4 text-center">
                All Book List
              </h3>
                ...
                <tbody>
                  {books.map((book, index) => (
                    <tr key={book._id} className="h-8">
                      <td className="border border-rounded text-center">
                        {index + 1}
                      </td>
                      <td className="border border-rounded text-center">
                        {book.title}
                      ...
                      <td className="border border-rounded text-center">
                        <div className="flex justify-center">
                          <Link to={`/books/details/${book._id}`}>
                            <span className="bi bi-info text-info "></span>
                          </Link>
                          <Link to={`/books/edit/${book._id}`}>
                            <span className="bi bi-pencil-square text-warning mx-2"></span>
                          </Link>
